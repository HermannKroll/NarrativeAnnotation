...
class OrganismVocabulary(TargetVocabulary):
    ...
    def __init__(self):
        super().__init__(ORGANISM_TAGGER_VOCAB, ORGANISM,  entity_type_in_vocab=ORGANISM)
        # Ignoriere KEINE Target-Typen (wir wollen ALLE durchsuchen)
        self.ignored_target_types = {}
        # Filter lokal auf ORGANISM
        self.allowed_target_types = {'organism'}
        self.use_headings_as_key = False
        # Lade von target Endpunkt (nicht organism)
        self.url_path_kw = "target"
        self.url_params = {
            'format': 'json',
            'limit': 1000
        }
        self.json_data_kw = "targets"

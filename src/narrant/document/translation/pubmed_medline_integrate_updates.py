...
def pubmed_medline_merge_with_updates(medline_file, medline_updates_file, output):
    medline_ids = set([TaggedDocument(content).id for content in read_documents(medline_file)])
    logging.info(f'{len(medline_ids)} PMIDs in Medline')
    update_ids = set([TaggedDocument(content).id for content in read_documents(medline_updates_file)])
    ...
    with open(output, 'wt') as f_out:
        ...
        for document_content in read_documents(medline_file):
        ...
        for document_content in read_documents(medline_updates_file):
            f_out.write(document_content + "\n")
